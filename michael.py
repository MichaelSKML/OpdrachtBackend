...
def receptdetailsgefilterd():
  ...
  mycursor.execute("SELECT idtag_recept, tag_id, naamtag, recept.naam, recept.aantalsterren, recept.afbeelding FROM tag_recept INNER JOIN tag ON tag_id = idtag INNER JOIN recept ON recept.id = idtag_recept ORDER by naamtag, recept.aantalsterren DESC;")
